...
MG     = 4*(np.pi**2)
beta   = 2.05
params = np.array([MG,beta],dtype='float')
...
different_vt = np.array([v_y_1])

for i in np.arange(len(different_vt)):
    
    velocities[0]  = np.array([different_vt[i],v_x_1])
...
######################################################################
    ...
    #---- Plotting to extract d(theta)/dt
    plt.subplot(211)
    plt.title('$Tangential$ $Velocity$ $Plots$', fontsize = 16)
    plt.plot(times,r_dist)
    plt.ylabel(r'$Distance$ $From$ $Sun$ [$AU$]')
...
#---- Precession Plot
dtheta_dt = np.array([0.021,0.025,.028,.034,.043,.057])
initial_v = np.array([v_y_1,v_y_1+.1,v_y_1+.2,v_y_1+.3,v_y_1+.4,v_y_1+.5])

plt.scatter(initial_v,dtheta_dt)
plt.xlabel(r'$Initial\,v_{T}$',fontsize = 18)
plt.ylabel(r'$d\theta/dt$',fontsize = 18)
plt.title('$Precision Rate$ $vs$ $Tangential$ $Velocity$', fontsize=15)
plt.show()

plt.plot(coordinates[:,0],coordinates[:,1])
plt.xlabel(r'$x-pos$ [$AU$]',fontsize = 18)
plt.ylabel(r'$y-pos$ [$AU$]',fontsize = 18)
plt.title('$Precision Rate$ $vs$ $Tangential$ $Velocity$', fontsize=15)
plt.show()

theta_dt = []

for i in np.arange(len(dtheta_dt)):
    theta_dt.append(np.max(theta)/dtheta_dt[i])

plt.scatter(initial_v, theta_dt)
plt.xlabel(r'$Initial\,v_{T}$',fontsize = 18)
plt.ylabel(r'$d\theta/dt$',fontsize = 18)
plt.title('$Precision Rate$ $vs$ $Tangential$ $Velocity$', fontsize=15)
plt.show()

slope,intercept = np.polyfit(initial_v,theta_dt,1)
print slope
